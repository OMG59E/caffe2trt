...
namespace nvcaffeparser1 {
    ILayer* parseReduction(INetworkDefinition& network, const trtcaffe::LayerParameter& msg,
        ...
        if (axis < 0 || axis > 3) {
            LOG(ERROR) << "Caffe Parser: Invalid axis in reduction layer - can only reduce NCHW input.";
            return nullptr;
        }

        if (network.hasImplicitBatchDimension() && axis == 0) {
            LOG(ERROR) << "Caffe Parser: Invalid axis in reduction layer - cannot reduce over batch size dimension.";
            return nullptr;
        }
...
#define GIE_3111 0
        ...
        // output a warning
        LOG(WARNING) << "Warning: The Reduce layer does not discard reduced dimensions. "
                        "The reduced dimensions are treated as dimensions of size one in the output of the Reduce layer.";
#endif
        input = returnVal->getOutput(0);
