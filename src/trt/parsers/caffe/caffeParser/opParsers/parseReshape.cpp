...
namespace nvcaffeparser1 {
    ILayer* parseReshape(INetworkDefinition& network, const trtcaffe::LayerParameter& msg,
        ...
        if (network.hasImplicitBatchDimension() && (axis == 0) && (shape.dim(0) != 0)) {
            LOG(ERROR) << "Caffe Parser: Invalid reshape param."
                          "TensorRT does not support reshape in N (batch) dimension";
            return nullptr;
        }
        ...
        for (int i = 0; i < shape.dim_size(); i++) {
            ...
            if (shape.dim(i) == 0) {
                ...
                LOG_ASSERT(static_cast<int>(reshapeDims.size()) <= bottomDims.nbDims);
        ...
        if (countMinusOne > 1) {
            LOG(ERROR) << "Caffe Parser: Invalid reshape param."
                          " At most one axis can be inferred from other dimensions";
            return nullptr;
        }
        ...
        DLOG(INFO) << "nbDims: " << topDims.nbDims << " Dims: "
                << topDims.d[0] << " " << topDims.d[1] << " " << topDims.d[2];
