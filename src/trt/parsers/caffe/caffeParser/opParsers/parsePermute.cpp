...
namespace nvcaffeparser1 {
    ILayer* parsePermute(INetworkDefinition& network, const trtcaffe::LayerParameter& msg,
        ...
        if (orderAbort) {
            LOG(ERROR) << "Caffe Parser: Invalid permute param."
                          " TensorRT does not support permute in N (batch) dimension, "
                          "and order index must be within the tensor dimensions. "
        ...
        LOG_ASSERT(parserutils::volume(topDims) == parserutils::volume(bottomDims));
