...
namespace nvcaffeparser1 {
    ILayer* parseSoftMax(INetworkDefinition& network, const trtcaffe::LayerParameter& msg,
        ...
        if (network.hasImplicitBatchDimension() && axis == 0) {
            LOG(ERROR) << "Caffe Parser: Invalid axis in softmax layer - TensorRT does not support softmax across the batch "
                         "axis with implicit batch dimensions networks.";
            return nullptr;
        }

        if (axis < 0 || axis > 3 || (axis > nbDims)) {
            LOG(ERROR) << "Caffe Parser: Invalid axis in softmax layer - TensorRT expects NCHW input. Negative axis is not "
