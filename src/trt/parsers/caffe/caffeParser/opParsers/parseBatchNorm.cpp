...
namespace nvcaffeparser1 {
    ...
    inline bool bnConvertWrap(float scaleFactor, const Weights& variance, const Weights& mean,
        ...
        LOG_ASSERT(shift.count == scale.count);
        if (nvCaffe) {
    ...
    ILayer* parseBatchNormalization(INetworkDefinition& network, const trtcaffe::LayerParameter& msg,
        ...
        if (nvCaffe) {
            ...
            LOG_ASSERT(mean.count == variance.count && movingAverage.count == 1);
        }
        ...
        if (!nvCaffe) {
            ...
            if (average == 0.0f) {
                LOG(WARNING) << "Batch normalization moving average is zero";
                return nullptr;
            }
            scaleFactor /= average;
        }
        if (mean.type == DataType::kFLOAT) {
            success = bnConvertWrap<float>(scaleFactor, variance,
                    mean, scaleBlob, biasBlob, shift, scale, p.eps(), nvCaffe, weightFactory);
        } else {
            success = bnConvertWrap<float16>(scaleFactor, variance,
                    mean, scaleBlob, biasBlob, shift, scale, p.eps(), nvCaffe, weightFactory);
        }
