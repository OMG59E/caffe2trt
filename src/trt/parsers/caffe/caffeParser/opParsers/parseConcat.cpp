...
namespace nvcaffeparser1 {
    ILayer* parseConcat(INetworkDefinition& network, const trtcaffe::LayerParameter& msg,
        ...
        if (hasAxis && p.axis() < 0) {
            LOG(ERROR) << "Caffe parser: Concat negative axis is not supported.";
            return nullptr;
        }
        if (network.hasImplicitBatchDimension() && p.axis() == 0) {
            LOG(ERROR) << "Caffe parser: Concat across batch axis with implicit batch dimensions is not supported.";
            return nullptr;
        }
