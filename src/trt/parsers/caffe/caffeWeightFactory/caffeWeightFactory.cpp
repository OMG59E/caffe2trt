...
void* convertInternal(void** ptr, int64_t count, bool* mOK) {
    ...
    for (int i = 0; i < count; ++i) {
        if (static_cast<OUTPUT>(iPtr[i]) > std::numeric_limits<OUTPUT>::max()
            ...
            LOG(ERROR) << "Error: Weight " << iPtr[i] << " is outside of [" << std::numeric_limits<OUTPUT>::max()
...
Weights CaffeWeightFactory::operator()(const std::string& layerName, WeightType weightType) {
    ...
    if (!blobMsg) {
        LOG(ERROR) << "Weights for layer " << layerName << " doesn't exist";
...
trtcaffe::Type CaffeWeightFactory::getBlobProtoDataType(const trtcaffe::BlobProto& blobMsg) {
    if (blobMsg.has_raw_data()) {
        LOG_ASSERT(blobMsg.has_raw_data_type());
...
std::pair<const void*, size_t> CaffeWeightFactory::getBlobProtoData(
        const trtcaffe::BlobProto& blobMsg, trtcaffe::Type type, std::vector<void*>& tmpAllocs) {
    ...
    const auto count = blobMsg.has_raw_data() ? blobMsg.raw_data().size() / sizeOfCaffeType(blobMsg.raw_data_type())
...
bool CaffeWeightFactory::checkForNans(const void* values, int count, const std::string& layerName) {
    ...
    for (int i = 0; i < count; i++) {
        if (std::isnan(float(v[i]))) {
            LOG(ERROR) << layerName << ": Nan detected in weights";
            return false;
        }
    }
    return true;
...
Weights CaffeWeightFactory::getWeights(const trtcaffe::BlobProto& blobMsg, const std::string& layerName) {
    ...
    if (blobProtoData.first == nullptr) {
        ...
        LOG(ERROR) << layerName << ": ERROR - " << bits << "-bit weights not found for "
                  << bits << "-bit model";
        mOK = false;
