...
#define RETURN_AND_LOG_ERROR_IMPL(ret, message, parserName)                                             \
    ...
    {                                                                                                   \
        ...
        if (getLogger())                                                                                \
            getLogger()->log(nvinfer1::ILogger::Severity::kERROR, errorMsg.c_str());                    \
        else                                                                                            \
            LOG(WARNING) << "WARNING: Logger not found, logging to stderr.\n" << errorMsg;              \
        return (ret);                                                                                   \
    } while (0)
    ...
    inline void printMem(const char *where) {
        ...
        LOG(INFO) << "   (memory) " << where << " : Free(MB) = " << avPages * pageSize / mb << " total(MB)="
                  << pages * pageSize / mb;
        ...
        LOG(INFO) << "   (memory) " << where << " : Free(MB) = " << statex.ullAvailPhys / mb << " total(MB)=" << statex.ullTotalPhys / mb;
#endif
    }
